    ...
    parameters: {
      ...
      properties: {
        intent: {
          ...
          description:
            "CREDIT_APPRAISAL: yêu cầu thẩm định một hồ sơ vay cụ thể (có dữ liệu khách hàng, khoản vay, thu nhập hoặc tài sản); dữ liệu thiếu sẽ được extraction model hỏi bổ sung, không ánh xạ sang hồ sơ mẫu. " +
...
Không ánh xạ nội dung sang case mẫu hoặc suy đoán dữ liệu hồ sơ. Việc nội dung thảo luận về
"prompt injection", bảo mật hay kiến trúc hệ thống không tự động biến nó thành một cuộc tấn công;
security gate riêng đã xử lý chỉ thị ghi đè thật trước bước phân loại này.
 ...
 * reach the credit-case router — that router's job is extracting a RetailCase,
